...
public class Piece2 : Piece {
    ...
    public new void OnEndDrag(PointerEventData eventData)
    {
        ...
        {
            botPosition = matches[0].transform.position;
        ...
        else
        {
            ...
            iTween.MoveTo(gameObject, iTween.Hash("position", botPosition, "speed", 15f, "oncomplete", "CompleteMoveToBottom"));
        }
    ...
    public new void ResetMatchColor()
    {
        //foreach (Tile tile in matches) tile.GetComponent<Image>().SetColorAlpha(1);
    }

    public new void HighlightMatchColor()
    {
        //foreach (Tile tile in matches) tile.GetComponent<Image>().SetColorAlpha(0f);
    }

    private void CompleteMoveToBoard()
    {
        transform.SetParent(MonoUtils.instance.bottomRegion);
